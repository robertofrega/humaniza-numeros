function humanizaNumeros(input, decimals = 0) {
  ...
  const bigNumPrefixes = ['Trilhões', 'Bilhões', 'Milhões', 'Mil'];

  // small numbers
  if (unsignedNumber < 1000) {
    return `${ signString }${ input.toFixed(decimals).replace('.', ',') }`;
  }
  ...
  const decimalPartArray = unsignedNumberCharacterArray.slice(decimalIndex, decimalIndex + decimals + 2);

  const wholePart = wholePartArray.join('');

  // pad decimalPart if necessary
  let decimalPart = parseFloat('0.' + decimalPartArray.join('')).toFixed(decimals).slice(2);
